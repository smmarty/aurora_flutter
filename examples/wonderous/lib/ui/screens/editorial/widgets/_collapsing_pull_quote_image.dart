...
class _CollapsingPullQuoteImage extends StatelessWidget {
  const _CollapsingPullQuoteImage(
      {Key? key, required this.scrollPos, required this.data})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    Widget buildText(String value, double collapseAmt,
        {required bool top, bool isAuthor = false}) {
    ...
    return ValueListenableBuilder<double>(
      ...
      builder: (context, value, __) {
        ...
        if (yPos != null && yPos < collapseStartPx) {
          ...
          collapseAmt = (collapseStartPx - max(collapseEndPx, yPos)) /
              (collapseStartPx - collapseEndPx);
        }
                      ...
                      child: StaticTextScale(
                        child: Column(
                          ...
                          children: [
                            ...
                            buildText(data.pullQuote1Top, collapseAmt,
                                top: true),
                            buildText(data.pullQuote1Bottom, collapseAmt,
                                top: false),
                            if (data.pullQuote1Author.isNotEmpty) ...[
                              Container(
                                margin: const EdgeInsets.only(top: 16),
                                child: buildText(
                                    '- ${data.pullQuote1Author}', collapseAmt,
                                    top: false, isAuthor: true),
                              )
                            ],
                          ],
                        ),
